...
struct is_data<
    Data, std::enable_if_t<
              ...
              std::is_same<decltype(data_nblocks(std::declval<const Data&>())), std::ptrdiff_t>::value &&
              std::is_same<decltype(data_blocksize(std::declval<const Data&>())), std::ptrdiff_t>::value &&
              std::is_same<decltype(data_stride(std::declval<const Data&>())), std::ptrdiff_t>::value &&
              std::is_same<decltype(data_count(std::declval<const Data&>())), std::ptrdiff_t>::value &&
...
void copy(const DataIn& src, const DataOut& dest) {
  ...
  if (data_iscontiguous(dest) && data_iscontiguous(src)) {
    ...
    if (data_iscontiguous(dest)) {
      ...
      for (std::ptrdiff_t i_block = 0; i_block < data_nblocks(src); ++i_block) {
      ...
      for (std::ptrdiff_t i_block = 0; i_block < data_nblocks(dest); ++i_block) {
      ...
      for (std::ptrdiff_t i = 0; i < data_count(src); ++i) {
