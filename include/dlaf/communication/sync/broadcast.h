...
void send(Communicator& communicator, DataIn&& message_to_send) {
  ...
  DLAF_MPI_CALL(MPI_Bcast(const_cast<DataT*>(message.data()), message.count(), message.mpi_type(), communicator.rank(),
            communicator));
}
...
void receive_from(const int broadcaster_rank, Communicator& communicator, DataOut&& data) {
  ...
  DLAF_MPI_CALL(MPI_Bcast(message.data(), message.count(), message.mpi_type(), broadcaster_rank, communicator));
}
}
}
