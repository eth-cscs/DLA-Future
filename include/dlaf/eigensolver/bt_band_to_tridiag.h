...
/// @pre @p mat_hh has block size (NB x NB)
/// @pre @p mat_hh has tile size (NB x NB)
///
...
/// @pre @p mat_e has block size (NB x MB)
/// @pre @p mat_e has tile size (NB x MB)
///
/// @param band_size size of the reflectors (normal one, not constrained by any matrix size limit)
/// @pre @p band_size is a divisor of `mat_hh.tile_size().cols()`
...
void bt_band_to_tridiagonal(const SizeType band_size, MatrixRef<T, D>& mat_e,
  ...
  DLAF_ASSERT(matrix::square_block_size(mat_hh), mat_hh);
