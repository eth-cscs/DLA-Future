...
///
/// @param[in] first_eigenvalue_index indicates the first index of the eigenvalues to compute
/// @param[in] last_eigenvalue_index indicates the last index of the eigenvalues to compute
...
void hermitian_eigensolver(blas::Uplo uplo, Matrix<T, D>& mat, Matrix<BaseType<T>, D>& eigenvalues,
  ...
  hermitian_eigensolver(uplo, mat, eigenvalues, eigenvectors, 0, mat.size().rows() - 1);
}
...
/// @param[in] last_eigenvalue_index indicates the last index of the eigenvalues to compute
template <Backend B, Device D, class T>
EigensolverResult<T, D> hermitian_eigensolver(blas::Uplo uplo, Matrix<T, D>& mat, SizeType first_eigenvalue_index,
                                              SizeType last_eigenvalue_index) {
  ...
  hermitian_eigensolver<B, D, T>(uplo, mat, eigenvalues, eigenvectors, first_eigenvalue_index, last_eigenvalue_index);
...
EigensolverResult<T, D> hermitian_eigensolver(blas::Uplo uplo, Matrix<T, D>& mat) {
  return hermitian_eigensolver<B, D, T>(uplo, mat, 0, mat.size().rows() - 1);
}
...
void hermitian_eigensolver(comm::CommunicatorGrid& grid, blas::Uplo uplo, Matrix<T, D>& mat,
  ...
  eigensolver::internal::Eigensolver<B, D, T>::call(grid, uplo, mat, eigenvalues, eigenvectors, 0, mat.size().rows() - 1);
}
