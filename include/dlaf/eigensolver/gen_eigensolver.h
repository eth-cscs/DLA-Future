...
void hermitian_generalized_eigensolver_helper(
    ...
    Matrix<T, D>& eigenvectors, const Factorization factorization, SizeType first_eigenvalue_index,
...
EigensolverResult<T, D> hermitian_generalized_eigensolver_helper(
    comm::CommunicatorGrid& grid, blas::Uplo uplo, Matrix<T, D>& mat_a, Matrix<T, D>& mat_b,
    const Factorization factorization, SizeType first_eigenvalue_index, SizeType last_eigenvalue_index) {
  DLAF_ASSERT(matrix::equal_process_grid(mat_a, grid), mat_a, grid);
  DLAF_ASSERT(matrix::equal_process_grid(mat_b, grid), mat_b, grid);
  DLAF_ASSERT(matrix::square_size(mat_a), mat_a);
  DLAF_ASSERT(matrix::square_blocksize(mat_a), mat_a);
...
EigensolverResult<T, D> hermitian_generalized_eigensolver(blas::Uplo uplo, Matrix<T, D>& mat_a,
  ...
  return eigensolver::internal::hermitian_generalized_eigensolver_helper<B, D, T>(
      uplo, mat_a, mat_b, Factorization::do_factorization, first_eigenvalue_index,
      last_eigenvalue_index);
}
...
EigensolverResult<T, D> hermitian_generalized_eigensolver(blas::Uplo uplo, Matrix<T, D>& mat_a,
                                                          Matrix<T, D>& mat_b) {
  return hermitian_generalized_eigensolver<B, D, T>(uplo, mat_a, mat_b, 0, mat_a.size().rows() - 1);
}
...
void hermitian_generalized_eigensolver_factorized(
    blas::Uplo uplo, Matrix<T, D>& mat_a, Matrix<T, D>& mat_b, Matrix<BaseType<T>, D>& eigenvalues,
    Matrix<T, D>& eigenvectors, SizeType first_eigenvalue_index, SizeType last_eigenvalue_index) {
  using eigensolver::internal::Factorization;
...
EigensolverResult<T, D> hermitian_generalized_eigensolver_factorized(
    blas::Uplo uplo, Matrix<T, D>& mat_a, Matrix<T, D>& mat_b, SizeType first_eigenvalue_index,
    SizeType last_eigenvalue_index) {
  using eigensolver::internal::Factorization;

  return eigensolver::internal::hermitian_generalized_eigensolver_helper<B, D, T>(
      uplo, mat_a, mat_b, Factorization::already_factorized, first_eigenvalue_index,
      last_eigenvalue_index);
}
...
EigensolverResult<T, D> hermitian_generalized_eigensolver(comm::CommunicatorGrid& grid, blas::Uplo uplo,
  ...
  return eigensolver::internal::hermitian_generalized_eigensolver_helper<B, D, T>(
      grid, uplo, mat_a, mat_b, Factorization::do_factorization, first_eigenvalue_index,
      last_eigenvalue_index);
}
...
void hermitian_generalized_eigensolver_factorized(
    comm::CommunicatorGrid& grid, blas::Uplo uplo, Matrix<T, D>& mat_a, Matrix<T, D>& mat_b,
    Matrix<BaseType<T>, D>& eigenvalues, Matrix<T, D>& eigenvectors, SizeType first_eigenvalue_index,
    SizeType last_eigenvalue_index) {
  using eigensolver::internal::Factorization;
  eigensolver::internal::hermitian_generalized_eigensolver_helper<B, D, T>(
...
void hermitian_generalized_eigensolver_factorized(comm::CommunicatorGrid& grid, blas::Uplo uplo,
  ...
  hermitian_generalized_eigensolver_factorized<B, D, T>(grid, uplo, mat_a, mat_b, eigenvalues,
                                                        eigenvectors, 0, mat_a.size().rows() - 1);
}
...
EigensolverResult<T, D> hermitian_generalized_eigensolver_factorized(
  ...
  return eigensolver::internal::hermitian_generalized_eigensolver_helper<B, D, T>(
      grid, uplo, mat_a, mat_b, Factorization::already_factorized, first_eigenvalue_index,
      last_eigenvalue_index);
}
