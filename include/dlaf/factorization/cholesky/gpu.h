...
void potrf_diag_tile(Executor&& exec,
                     hpx::future<matrix::Tile<T, Device::GPU>> matrix_tile) {
...
void trsm_panel_tile(Executor&& executor_hp,
                     hpx::shared_future<matrix::Tile<const T, Device::GPU>> kk_tile,
                     hpx::future<matrix::Tile<T, Device::GPU>> matrix_tile) {
...
void herk_trailing_diag_tile(Executor&& trailing_matrix_executor,
                             hpx::shared_future<matrix::Tile<const T, Device::GPU>> panel_tile,
                             hpx::future<matrix::Tile<T, Device::GPU>> matrix_tile) {
...
void gemm_trailing_matrix_tile(Executor&& trailing_matrix_executor,
                               hpx::shared_future<matrix::Tile<const T, Device::GPU>> panel_tile,
                               hpx::shared_future<matrix::Tile<const T, Device::GPU>> col_panel,
                               hpx::future<matrix::Tile<T, Device::GPU>> matrix_tile) {
...
template <class T>
struct Cholesky<Backend::GPU, Device::GPU, T> {
  static void call_L(Matrix<T, Device::GPU>& mat_a);
  static void call_L(comm::CommunicatorGrid grid, Matrix<T, Device::GPU>& mat_a);
};
...
void Cholesky<Backend::GPU, Device::GPU, T>::call_L(comm::CommunicatorGrid grid,
  ...
  for (SizeType k = 0; k < nrtile; ++k) {
    ...
    if (this_rank == kk_rank) {
      potrf_diag_tile(executor_lapack, mat_a(kk_idx));
      ...
      if (k != nrtile - 1)
        panel[k] = comm::recv_tile<T, Device::GPU>(executor_mpi, mpi_task_chain, Coord::Col,
    ...
    for (SizeType i = k + 1; i < nrtile; ++i) {
      ...
      if (this_rank == ik_rank) {
        ...
        panel[i] = comm::recv_tile<T, Device::GPU>(executor_mpi, mpi_task_chain, Coord::Row,
    ...
    for (SizeType j = k + 1; j < nrtile; ++j) {
      ...
      if (this_rank.row() == jj_rank.row()) {
        ...
        if (j != nrtile - 1)
          panel[j] = comm::recv_tile<T, Device::GPU>(executor_mpi, mpi_task_chain, Coord::Col,
