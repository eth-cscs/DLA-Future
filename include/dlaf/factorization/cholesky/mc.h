...
void Cholesky<Backend::MC, Device::CPU, T>::call_L(comm::CommunicatorGrid grid,
  ...
  for (SizeType k = 0; k < nrtile; ++k) {
    ...
    if (this_rank == kk_rank) {
      ...
      if (k != nrtile - 1)
        panel[k] = dataflow(executor_mpi, comm::recvAllocTile<T, Device::CPU>, mpi_task_chain(),
                            Coord::Col, mat_a.tileSize(kk_idx), kk_rank.row());
    }
    ...
    for (SizeType i = k + 1; i < nrtile; ++i) {
      ...
      if (this_rank == ik_rank) {
        ...
        panel[i] = dataflow(executor_mpi, comm::recvAllocTile<T, Device::CPU>, mpi_task_chain(),
                            Coord::Row, mat_a.tileSize(ik_idx), ik_rank.col());
      }
    }
    ...
    for (SizeType j = k + 1; j < nrtile; ++j) {
      ...
      if (this_rank.row() == jj_rank.row()) {
        ...
        if (j != nrtile - 1)
          panel[j] = dataflow(executor_mpi, comm::recvAllocTile<T, Device::CPU>, mpi_task_chain(),
                              Coord::Col, mat_a.tileSize(jk_idx), jj_rank.row());
      }
