...
void Triangular<backend, D, T>::call_L(comm::CommunicatorGrid& grid, blas::Diag diag,
  ...
  fname << "triangular-inverse-L-" << matrix::internal::TypeToString_v<T> << "-"
    ...
    if (k < nrtiles - 1) {
      ...
      if (kk_rank.row() == this_rank.row()) {
        for (SizeType j_local = 0; j_local < k1_local_col; ++j_local) {
          ...
          row_panel.setTile(kj_panel, mat_a.read(kj_local));
        }
      }
    ...
    if (k > 0) {
      ...
      if (kk_rank.col() == this_rank.col()) {
        for (SizeType i_local = k_local_row; i_local < i_local_end; ++i_local) {
          ...
          col_panel.setTile(ik_panel, mat_a.read(ik_local));
        }
      }
...
void Triangular<backend, D, T>::call_U(comm::CommunicatorGrid& grid, blas::Diag diag,
  ...
  static std::atomic<size_t> num_triangular_inv_calls = 0;
  std::stringstream fname;
  fname << "triangular-inverse-U-" << matrix::internal::TypeToString_v<T> << "-"
        << std::to_string(num_triangular_inv_calls) << ".h5";
  std::optional<matrix::internal::FileHDF5> file;
  ...
  for (SizeType k = nrtiles - 1; k >= 0; --k) {
    ...
    if (k < nrtiles - 1) {
      col_panel.setRange({0, 0}, {k + 1, k + 1});
      if (k == 0)
        col_panel.setWidth(mat_a.tile_size_of(kk).cols());
    ...
    if (kk_rank.row() == this_rank.row()) {
      for (SizeType j_local = k1_local_col; j_local < j_local_end; ++j_local) {
        const LocalTileIndex kk_local_panel{Coord::Row, k_local_row};
        const LocalTileIndex kj_local{k_local_row, j_local};
        trsm_row_panel_tile<backend>(thread_priority::high, diag, col_panel.read(kk_local_panel),
                                     mat_a.readwrite(kj_local));
    ...
    if (k > 0) {
      row_panel.setRangeStart(kk);
      if (k == nrtiles - 1)
        row_panel.setHeight(mat_a.tile_size_of(kk).cols());

      if (kk_rank.row() == this_rank.row()) {
        for (SizeType j_local = k_local_col; j_local < j_local_end; ++j_local) {
          const LocalTileIndex kj_panel(Coord::Col, j_local);
          const LocalTileIndex kj_local{k_local_row, j_local};
    ...
    for (SizeType j_local = k1_local_col; j_local < j_local_end; ++j_local) {
      for (SizeType i_local = 0; i_local < k_local_row; ++i_local) {
        ...
        const auto trailing_matrix_priority =
            (i_local == k_local_row - 1) ? thread_priority::high : thread_priority::normal;
