...
template <class MatrixLike>
  bool is_allocated_as_blocks(MatrixLike& mat) {
    ...
    for (SizeType j_bl = 0, j = 0; j_bl < local_nr_blocks.cols(); ++j_bl) {
      // When computing the number of tiles in block, special care is needed when onlny a single block is present,
      // as it can contain two incomplete tiles.
      ...
      for (SizeType i_bl = 0, i = 0; i_bl < local_nr_blocks.rows(); ++i_bl) {
        // When computing the number of tiles in block, special care is needed when only a single block is present,
        // as it can contain two incomplete tiles.
