  ...
  if constexpr (D == Device::CPU) {
    auto permute_fn = [subm_dist](const auto& index_tile_futs, const auto& mat_in_tiles,
      ...
      pika::for_loop(pika::execution::par, to_sizet(0), to_sizet(nperms), [&](SizeType i_perm) {
        ...
        applyPermutation<T, C>(i_perm, splits, out_begin, subm_dist.size(), in_offset, subm_dist,
                               perm_arr, mat_in_tiles, mat_out_tiles);
      });
    };
...
void applyPackingIndex(const matrix::Distribution& subm_dist, IndexMapSender&& index_map, InSender&& in,
  ...
  if constexpr (D == Device::CPU) {
    ...
    auto permute_fn = [subm_dist](const auto& index_tile_futs, const auto& mat_in_tiles,
      ...
      ex::start_detached(dlaf::internal::transform(dlaf::internal::Policy<Backend::GPU>(),
                                                   std::move(permute_fn), std::move(sender)));
    };
#endif
  }
...
void unpackLocalOnCPU(const matrix::Distribution& subm_dist, const matrix::Distribution& dist,
  ...
  auto permutations_unpack_local_f = [subm_dist](const auto i_perm, const auto& args) {
    ...
    if (a <= perm_arr[i_perm] && perm_arr[i_perm] < b) {
      applyPermutation<T, C>(i_perm, splits, {0, 0}, sz, 0, subm_dist, perm_arr, mat_in_tiles,
                             mat_out_tiles);
    }
  };
