...
struct Solver<Backend::GPU> {
  ...
  static void triangular(comm::CommunicatorGrid grid, blas::Side side, blas::Uplo uplo, blas::Op op,
                         blas::Diag diag, T alpha, Matrix<const T, Device::GPU>& mat_a,
                         Matrix<T, Device::GPU>& mat_b);
};

}

#include <dlaf/solver/triangular/gpu.tpp>
...
#define DLAF_TRIANGULAR_ETI(KWORD, DATATYPE) \
  KWORD template void Solver<Backend::MC>::triangular<DATATYPE>(blas::Side, blas::Uplo, blas::Op, \
                                                                blas::Diag, DATATYPE, \
                                                                Matrix<const DATATYPE, Device::GPU>&, \
                                                                Matrix<DATATYPE, Device::GPU>&);
