...
template <class T>
void triangular_RLN(blas::Diag diag, T alpha, Matrix<const T, Device::CPU>& mat_a, Matrix<T, Device::CPU>& mat_b) {
  ...
  for (SizeType k = n - 1; k > -1; --k) {
    ...
    for (SizeType i = m - 1; i > -1; --i) {
      ...
      hpx::dataflow(executor_hp, hpx::util::unwrapping(tile::trsm<T, Device::CPU>), Right, Lower, NoTrans,
...
// Distributed implementation of Right Lower NoTrans
template <class T>
  void triangular_RLN(comm::CommunicatorGrid grid, blas::Diag diag, T alpha, Matrix<const T, Device::CPU>& mat_a, Matrix<T, Device::CPU>& mat_b) {
  constexpr auto Right = blas::Side::Right;
  constexpr auto Lower = blas::Uplo::Lower;
  constexpr auto NoTrans = blas::Op::NoTrans;
