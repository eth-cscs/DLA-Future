...
namespace memory{
    ...
    public:
        ...
        Host(std::size_t size, Allocator alloc = Allocator())
        : size_(size), ptr_(nullptr), alloc_(alloc) {
            #ifdef WITH_CUDA
        ...
        // copy constructor
        Host(const Host& rhs)
        : size_(rhs.size_), ptr_(nullptr), alloc_(rhs.alloc_) {
            #ifdef WITH_CUDA
            cudaMallocHost(&ptr_, size_ * sizeof(T));
            #else
            ptr_ = alloc_.allocate(size_);
        ...
        // move constructor
        Host(Host&& rhs)
        : size_(rhs.size_), ptr_(rhs.ptr_), alloc_(rhs.alloc_) {
            rhs.ptr_ = nullptr;
        }
