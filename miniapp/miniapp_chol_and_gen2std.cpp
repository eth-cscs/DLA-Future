...
struct Options
  ...
  Options(const pika::program_options::variables_map& vm)
      : MiniappOptions(vm), m(vm["matrix-size"].as<SizeType>()), mb(vm["block-size"].as<SizeType>()),
        ...
        runSerially(vm["sequential"].as<bool>()) {
    DLAF_ASSERT(m > 0, m);
    DLAF_ASSERT(mb > 0, mb);
...
struct GenToStdMiniapp {
  ...
  static void run(const Options& opts) {
    ...
    for (int64_t run_index = -opts.nwarmups; run_index < opts.nruns; ++run_index) {
      ...
      double gigaflops;
      {
        const double n = matrix_a_host.size().rows();
        const auto add_mul = n * n * n / 6 + n * n * n / 2;
...
int main(int argc, char** argv) {
  ...
  desc_commandline.add_options()
    ("sequential,s", bool_switch()      ->default_value(false), "Run algorithms sequentially")
