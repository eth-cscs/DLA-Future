...
struct TriangularInverseMiniapp {
  ...
  static void run(const Options& opts) {
    ...
    for (int64_t run_index = -opts.nwarmups; run_index < opts.nruns; ++run_index) {
      ...
      double elapsed_time;
      {
        ...
        if (opts.local)
          dlaf::triangular_inverse<backend, DefaultDevice_v<backend>, T>(opts.uplo, opts.diag,
                                                                         matrix.get());
        else
          dlaf::triangular_inverse<backend, DefaultDevice_v<backend>, T>(comm_grid, opts.uplo, opts.diag,
                                                                         matrix.get());
...
int pika_main(pika::program_options::variables_map& vm) {
  ...
  dlaf::miniapp::dispatchMiniapp<TriangularInverseMiniapp>(opts);
...
int main(int argc, char** argv) {
  ...
  options_description desc_commandline("Usage: miniapp_triangular_inverse [options]");
