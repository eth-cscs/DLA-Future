...
void work0(Wrapper<Tile<int>>&& i, int index) {
  auto& i_ = i.get()();
...
// Sum j to tile value.
void work(Wrapper<Tile<int>>&& i, int j, int index) {
  auto& i_ = i.get()();
...
void work2(Wrapper<Tile<int>>&& i, const Wrapper<Tile<int>>& j, int index) {
  auto& i_ = i.get()();
  auto& j_ = j.get()();
...
void foo() {
  ...
  std::array<hpx::future<Type>, 4> fa = {
      hpx::make_ready_future<Type>(&a[0]), hpx::make_ready_future<Type>(&a[1]),
      hpx::make_ready_future<Type>(&a[2]), hpx::make_ready_future<Type>(&a[3])};

  std::array<hpx::future<Type>, 4> fb = {
      hpx::make_ready_future<Type>(&b[0]), hpx::make_ready_future<Type>(&b[1]),
      hpx::make_ready_future<Type>(&b[2]), hpx::make_ready_future<Type>(&b[3])};

  Matrix<Type> ma(std::move(fa));
  Matrix<Type> mb(std::move(fb));
  // end of matrices setup

  // execute some operation on matrix B asynchronously.
  MatrixRW<Type> mb1 = mb.block();
  ...
  MatrixRW<Type> ma1 = ma.block();
  MatrixRead<Type> mb2 = mb.block_read();
  hpx::async([ma1 = std::move(ma1), mb2 = std::move(mb2), &HP]() mutable {
    for (std::size_t i = 1; i < 4; ++i) {
      hpx::dataflow(hpx::util::unwrapping(work2<>), std::move(ma1(0)), std::move(ma1.read(i)),
                    100 + i);
    }
    {
    ...
    for (std::size_t i = 0; i < 4; ++i) {
      hpx::dataflow(hpx::util::unwrapping(work2<>), std::move(ma1(i)), std::move(mb2.read(i)),
                    300 + i);
