...
SCRATCH=$( pwd )
GIT_DIR=$HOME/src/dlaf/
GCC=11.2.0
ARCH=cray-cnl7-haswell
ARCH=linux-pop22-skylake
...
for configuration in "${configurations[@]}"; do
    ...
    if [[ "${configuration}" == "mc" ]]; then
        spec="dla-future%gcc@$GCC arch=$ARCH build_type=Release ~cuda ^intel-mkl"
    ...
    paths_array=()
    for commitish in "$@"; do
        commit=$(git -C $GIT_DIR rev-parse ${commitish})

        # create worktree
        worktree_path=$SCRATCH/dlaf-comparisons/${commit}
        [[ ! -d "${worktree_path}" ]] && git -C $GIT_DIR worktree add "${worktree_path}" "${commit}"
        ...
        echo "spack command is $spack_cmd"

        spec_hash=$(${spack_cmd} spec --json ${spec} | jq --raw-output '.spec.nodes[] | select(.name == "dla-future") | .full_hash')
        echo "spec commmand ${spack_cmd} spec --json ${spec} | jq --raw-output '.spec.nodes[] | select(.name == \"dla-future\") | .full_hash'"
...
#        mkdir -p "${build_dir}"
#
        echo "spec commmand ${spack_cmd} spec --fresh -lI ${spec}"
        ${spack_cmd} spec --fresh -lI ${spec}
#        # install dependencies
#        ${spack_cmd} install --fresh --only dependencies ${spec}
#
#        # build dla-future
#        ${spack_cmd} -e dev build-env ${spec} -- cmake ${worktree_path} -B${build_dir} ${cmake_flags}
#        ${spack_cmd} -e dev build-env ${spec} -- cmake --build ${build_dir} --target miniapp_{cholesky,triangular_solver,gen_to_std}
#
