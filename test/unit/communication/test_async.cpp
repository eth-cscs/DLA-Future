...
TEST(Comm, SendRecv) {
  ...
  auto send_fut = dlaf::comm::async(ex, comm, MPI_Isend, send_buf.data(), size, dtype, send_rank, tag);
  auto recv_fut = dlaf::comm::async(ex, comm, MPI_Irecv, recv_buf.data(), size, dtype, recv_rank, tag);
