...
const std::vector<std::tuple<SizeType, SizeType, SizeType, SizeType>> sizes =
    {{3, 0, 1, 1},   {0, 5, 2, 3},  // m, n = 0
     ...
     {3, 3, 1, 1},   {4, 4, 1, 1},   {4, 4, 2, 2}, {6, 3, 3, 3}, {12, 2, 4, 4},
     {12, 24, 3, 3}, {24, 36, 6, 6}, {5, 8, 3, 2}, {4, 6, 2, 3}, {5, 5, 2, 3},
     {8, 27, 3, 4},  {15, 34, 4, 6}, {7, 6, 2, 3}, {8, 5, 2, 3}, {7, 5, 2, 3}};
...
void testBacktransformationEigenv(comm::CommunicatorGrid grid, SizeType m, SizeType n, SizeType mb,
  ...
  if (tottaus > 0) {
    ...
    for (const auto& ij_tile : iterate_range2d(mat_v_loc.nrTiles())) {
      if (ij_tile.row() <= ij_tile.col()) {
        tile::set0<T>(mat_v_loc.tile(ij_tile));
      }
      else if (ij_tile.row() == ij_tile.col() + 1) {
    ...
    setZero(mat_t);
    auto mat_t_loc = dlaf::matrix::test::allGather<T>(mat_t, grid);
