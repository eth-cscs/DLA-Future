...
TYPED_TEST(CholeskyLocalTest, Correctness) {
  ...
  for (const auto& size : square_sizes) {
    for (const auto& block_size : square_block_sizes) {
      ...
      Matrix<TypeParam, Device::CPU> mat_h(size, block_size);
      set(mat_h, el);

      {
        MatrixMirror<TypeParam, Device::GPU, Device::CPU> mat(mat_h);
        factorization::cholesky<Backend::GPU>(blas::Uplo::Lower, mat.get());
      }

      CHECK_MATRIX_NEAR(res, mat_h, 4 * (mat_h.size().rows() + 1) * TypeUtilities<TypeParam>::error,
                        4 * (mat_h.size().rows() + 1) * TypeUtilities<TypeParam>::error);
    }
  }
}
  ...
  for (const auto& comm_grid : this->commGrids()) {
    for (const auto& size : square_sizes) {
      for (const auto& block_size : square_block_sizes) {
        ...
        Matrix<TypeParam, Device::CPU> mat_h(std::move(distribution));
        set(mat_h, el);

        {
          MatrixMirror<TypeParam, Device::GPU, Device::CPU> mat(mat_h);
          factorization::cholesky<Backend::GPU>(comm_grid, blas::Uplo::Lower, mat.get());
        }
