...
TYPED_TEST(ExtraBuffersTest, FutureOrder) {
  ...
  matrix.read(LocalTileIndex{0, 0}).then(hpx::launch::sync, unwrapping([](auto&& tile) {
                                           EXPECT_EQ(TypeUtils::element(4, 0), tile({0, 0}));
                                         }));
  buffers.get_buffer(0).then(hpx::launch::sync, unwrapping([](auto tile) {
                               tile({0, 0}) += TypeUtils::element(1, 0);
                             }));
  buffers.get_buffer(0).then(hpx::launch::sync, unwrapping([](auto tile) {
                               tile({0, 0}) *= TypeUtils::element(2, 0);
                             }));
  auto f = matrix.read(LocalTileIndex{0, 0}).then(hpx::launch::sync, unwrapping([](auto&& tile) {
                                                    EXPECT_EQ(TypeUtils::element(4, 0), tile({0, 0}));
                                                  }));
  buffers.unlock_base();
  f.get();
