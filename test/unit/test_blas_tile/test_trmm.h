...
void testTrmm(const blas::Side side, const blas::Uplo uplo, const blas::Op op, const blas::Diag diag,
  ...
  if (side == blas::Side::Left)
    std::tie(el_op_a, res_b, el_b) = getLeftTriangularSystem<ElementIndex, T>(uplo, op, diag, static_cast<T>(1.0)/alpha, m);
  else
    std::tie(el_op_a, res_b, el_b) = getRightTriangularSystem<ElementIndex, T>(uplo, op, diag, static_cast<T>(1.0)/alpha, n);
